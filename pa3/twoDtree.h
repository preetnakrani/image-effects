...
private:
   ...
   */
   ...
   void render(PNG& edit, Node* subroot);

   void prune(Node* subtree, int tol);
   
   bool prunning(Node* subtree, int tol, RGBAPixel avgPixel);
   
   int pruneSize(Node* subtree, int tol);

   int countLeaves(Node* subtree);

   pair<int, int> getVS(stats& s, pair<int, int> ul, pair<int, int> lr);
   pair<int, int> getHS(stats& s, pair<int, int> ul, pair<int, int> lr);
   pair<int, int> getOther(bool vert, pair<int, int> ul, pair<int, int> lr);
