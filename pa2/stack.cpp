...
T Stack<T>::pop(){
    ...
    T thisItem = items[num_items - 1];
    num_items = num_items - 1;
    size_t shrinking = (max_items/SHRINKRATE);
    size_t expanding = (max_items/EXPANSIONFACTOR);
    
//    if  (num_items < shrinking && expanding >= DEFAULTCAPACITY) {
////        size_t resizeVal;
////        if (DEFAULTCAPACITY > shrinking) {
////            resizeVal = DEFAULTCAPACITY;
////        } else {
////            resizeVal = shrinking;
////        }
//        resize(expanding);
//    }
    if (num_items < shrinking) {
        if (expanding < DEFAULTCAPACITY) {
            expanding = DEFAULTCAPACITY;
        }
        resize(expanding);
    }
